...
/**
 * Holds a group of claim chunks that are all attached to each other. This will end up being a single rendered
 * claim in dynmap.
 */

public class GroupedChunks {
    private Map<ChunkPosition, AdjacentChunk> m_ChunkGroupMap = new HashMap<>();
    private Logger logger = WarsFtbDynmapMod.logger;
    ...
    public void processChunk(ChunkPosition chunkPos, Set<ChunkPosition> teamClaimsMap) {
        ...
        m_ChunkGroupMap.put(chunkPos, chunk);

        // Have each chunk find its adjacent chunks and process them recursively
        chunk.processAdjacentChunks(teamClaimsMap, m_ChunkGroupMap);
    }
    ...
    public List<PositionPoint> traceShapePerimeter() {
        ...
        for (Map.Entry<ChunkPosition, AdjacentChunk> chunk : m_ChunkGroupMap.entrySet()) {
        ...
        if (!pointSearchMap.isEmpty()) {
            ...
            do {
                ...
                if (curEdge != null) {
                    ...
                    logger.error("Unable to successfully trace claim chunk perimeter. This claim will not be visible on dynmap.");
                    bTraceError = true;
                    break;
                }
                ...
                if (nLoopLimit <= 0) {
                    logger.error("Unable to find starting point in claim trace. This claim will not be visible on dynmap.");
                    bTraceError = true;
                    break;
                }
